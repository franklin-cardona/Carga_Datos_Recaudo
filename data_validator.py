...
class BusinessRuleValidator:
    ...
    def _validate_positive_number(self, value: Any, **kwargs) -> Tuple[bool, Optional[str]]:
        ...
        positive_indicators = ['price', 'amount',
                               'quantity', 'count', 'total', 'precio', 'cantidad']
...
class DataValidator:
    ...
    def validate_dataframe(self, df: pd.DataFrame,
        ...
        try:
            ...
            if table_constraints:
                constraint_issues = self._validate_table_constraints(
                    df, column_mappings, table_constraints)
    ...
    def _validate_structure(self, df: pd.DataFrame,
        ...
        expected_columns = {
            mapping.excel_column for mapping in column_mappings}
    ...
    def _validate_table_constraints(self, df: pd.DataFrame,
        ...
        for mapping in column_mappings:
            if mapping.db_column in unique_columns and mapping.excel_column in df.columns:
                duplicates = df[df[mapping.excel_column].duplicated(
                    keep=False)]
            ...
            for idx, value in df[mapping.excel_column].items():
                for rule in applicable_rules:
                    try:
                        ...
                        self.logger.warning(
                            f"Error aplicando regla '{rule.name}': {str(e)}")
    ...
    def generate_validation_report(self, validation_result: ValidationResult) -> str:
        ...
        report.append(
            f"  Filas procesadas: {validation_result.processed_rows}")
        report.append(f"  Filas válidas: {validation_result.valid_rows}")
        report.append(
            f"  Errores encontrados: {len(validation_result.errors)}")
        report.append(f"  Advertencias: {len(validation_result.warnings)}")
        report.append(
            f"  Estado: {'VÁLIDO' if validation_result.is_valid else 'INVÁLIDO'}")
        report.append("")
        ...
        if validation_result.errors:
            ...
            for i, error in enumerate(validation_result.errors[:10], 1):
                report.append(
                    f"{i}. Fila {error['row']}, Columna '{error['column']}':")
            ...
            if len(validation_result.errors) > 10:
                report.append(
                    f"... y {len(validation_result.errors) - 10} errores más")
                report.append("")
        ...
        if validation_result.warnings:
            ...
            for i, warning in enumerate(validation_result.warnings[:5], 1):
                report.append(
                    f"{i}. Fila {warning['row']}, Columna '{warning['column']}':")
            ...
            if len(validation_result.warnings) > 5:
                report.append(
                    f"... y {len(validation_result.warnings) - 5} advertencias más")
                report.append("")
