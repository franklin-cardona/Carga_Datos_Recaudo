...
class LoginWindow:
    ...
    def _create_widgets(self):
        ...
        self.remember_check.grid(
            row=6, column=0, columnspan=2, sticky=tk.W, pady=(0, 20))

        # Botón de login
        self.login_button = ttk.Button(main_frame, text="Iniciar Sesión",
                                       command=self._on_login_click)
        self.login_button.grid(row=7, column=0, columnspan=2,
        ...
        info_frame = ttk.LabelFrame(
            main_frame, text="Información", padding="10")
        ...
        try:
            if not self.auth_manager:
                self.root.after(0, lambda: self._show_error(
                    "Error: Gestor de autenticación no configurado"))
                return
    ...
    def _set_loading_state(self, loading: bool):
        ...
        if loading:
            ...
            self.status_label.config(
                text="Verificando credenciales...", foreground="blue")
        else:
...
class LoginDialog:
    ...
    def _create_dialog_widgets(self):
        ...
        self.username_entry = ttk.Entry(
            main_frame, textvariable=self.username_var, width=30)
    ...
    def _login(self):
        ...
        try:
            ...
            if result['success']:
                ...
                messagebox.showerror(
                    "Error de Autenticación", result['message'])
                self.password_var.set("")
                self.password_entry.focus()
        except Exception as e:
