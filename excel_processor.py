...
class ExcelProcessor:
        ...
        try:
            ...
            for sheet_name in excel_file.sheet_names:
                try:
                    ...
                    self.logger.warning(f"Error leyendo hoja '{sheet_name}': {str(e)}")
                    worksheet_info[sheet_name] = {
                        'error': str(e),
                        'row_count': 0,
                        'column_count': 0,
            ...
            self.logger.error(f"Error obteniendo información de hojas: {str(e)}")
            raise
    ...
    def detect_column_types(self, df: pd.DataFrame) -> Dict[str, DataType]:
        ...
        for column in df.columns:
            ...
            for value in string_series:
                ...
                # Verificar número
                is_integer = re.match(self.number_patterns[0], value.replace(',', ''))
                if is_integer:
    ...
    def fuzzy_match_columns(self, excel_columns: List[str], 
        ...
        for i, excel_col in enumerate(excel_columns):
            ...
            for db_col in db_columns:
                ...
                partial_score = fuzz.partial_ratio(excel_col.lower(), db_col.lower())
                token_sort_score = fuzz.token_sort_ratio(excel_col.lower(), db_col.lower())
                token_set_score = fuzz.token_set_ratio(excel_col.lower(), db_col.lower())
    ...
    def map_worksheet_to_table(self, file_path: str, worksheet_name: str, 
        ...
        try:
            ...
            for table_name, table_columns in available_tables.items():
                ...
                name_score = fuzz.ratio(worksheet_name.lower(), table_name.lower())
                
                # Calcular similitud de columnas
                excel_columns = list(df.columns)
                column_mappings = self.fuzzy_match_columns(excel_columns, table_columns)
                
                # Calcular score promedio de mapeo de columnas
                if column_mappings:
                    avg_column_score = sum(m.confidence_score for m in column_mappings) / len(column_mappings)
                    combined_score = (name_score * 0.3 + avg_column_score * 100 * 0.7)
                else:
            ...
            if best_table:
                ...
                column_mappings = self.fuzzy_match_columns(list(df.columns), table_columns)
            ...
            self.logger.error(f"Error mapeando hoja '{worksheet_name}': {str(e)}")
            raise
    ...
    def validate_data(self, file_path: str, worksheet_mapping: WorksheetMapping) -> ValidationResult:
        ...
        try:
            ...
            df = pd.read_excel(file_path, sheet_name=worksheet_mapping.worksheet_name)
        ...
        for mapping in column_mappings:
            ...
            try:
                if mapping.data_type == DataType.INTEGER:
                    ...
                    df_converted[mapping.excel_column] = df_converted[mapping.excel_column].apply(convert_bool)
                    ...
                    df_converted[mapping.excel_column] = df_converted[mapping.excel_column].astype(str)
                    # Reemplazar 'nan' string con None
                    df_converted[mapping.excel_column] = df_converted[mapping.excel_column].replace('nan', None)
                
            except Exception as e:
                self.logger.warning(f"Error convirtiendo columna '{mapping.excel_column}': {str(e)}")
        ...
        try:
            ...
            for worksheet_name, info in worksheet_info.items():
                if not info.get('has_data', False):
                    self.logger.warning(f"Saltando hoja '{worksheet_name}' - sin datos")
                    continue
                ...
                try:
                    ...
                    self.logger.error(f"Error procesando hoja '{worksheet_name}': {str(e)}")
