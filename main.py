...
class ExcelSQLIntegrationApp:
    ...
    def _show_connection_dialog(self) -> bool:
        ...
        try:
            ...
            dialog = ConnectionDialog(on_connection_success=self._on_connection_configured)
            ...
            if config:
                ...
                self.logger.info("Usuario canceló la configuración de conexión")
                return False
                
        except Exception as e:
            self.logger.error(f"Error en diálogo de conexión: {str(e)}")
            messagebox.showerror("Error", f"Error en configuración de conexión:\n{str(e)}")
            return False
    ...
    def _on_connection_configured(self, config: Dict[str, Any]):
        ...
        """
        self.connection_config = config
        self.logger.info(f"Conexión configurada para servidor: {config['server']}")
    ...
    def _initialize_database_connection(self) -> bool:
        ...
        try:
            if not self.connection_config:
                self.logger.error("No hay configuración de conexión disponible")
                return False
            ...
            if not success:
                self.logger.error(f"Error de conexión a base de datos: {error_msg}")
            ...
            self.logger.info("Conexión a base de datos inicializada exitosamente")
            return True
    ...
    def _check_database_schema(self) -> bool:
        ...
        try:
            ...
            for table in required_tables:
                ...
                result = self.db_connection.execute_query(query, (schema, table_name))
    ...
    def _start_main_application(self):
        ...
        try:
            ...
            self.logger.error(f"Error iniciando aplicación principal: {str(e)}")
            messagebox.showerror("Error", f"Error en aplicación principal: {str(e)}")
    ...
    def _logout(self):
        ...
        try:
            if self.current_user and self.current_user['session_id']:
                ...
                self.logger.info(f"Sesión cerrada para usuario: {self.current_user['username']}")
    ...
    def run(self):
        ...
        try:
            ...
            if not self._initialize_database_connection():
                self.logger.error("No se pudo inicializar la conexión a base de datos")
                return
            
            # Paso 3: Verificar esquema de base de datos
            if not self._check_database_schema():
                self.logger.info("Verificación de esquema cancelada por el usuario")
                return
