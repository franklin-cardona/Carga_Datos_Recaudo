...
class ConnectionDialog:
    ...
    def __init__(self, parent=None, on_connection_success: Callable[[Dict[str, Any]], None] = None):
        ...
        self.database_var = tk.StringVar(value="ExcelSQLIntegration")
    ...
    def _create_widgets(self):
        ...
        # Entorno
        ttk.Label(config_frame, text="Entorno:").grid(row=0, column=0, sticky=tk.W, pady=(0, 5))
        ...
        # Servidor
        ttk.Label(config_frame, text="Servidor:").grid(row=2, column=0, sticky=tk.W, pady=(0, 5))
        self.server_entry = ttk.Entry(config_frame, textvariable=self.server_var, width=40)
        self.server_entry.grid(row=3, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 15))
        
        # Base de datos
        ttk.Label(config_frame, text="Base de Datos:").grid(row=4, column=0, sticky=tk.W, pady=(0, 5))
        ...
        auth_frame = ttk.LabelFrame(config_frame, text="Autenticación", padding="10")
        auth_frame.grid(row=6, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 15))
        ...
        ttk.Label(self.credentials_frame, text="Usuario:").grid(row=0, column=0, sticky=tk.W, pady=(0, 5))
        self.username_entry = ttk.Entry(self.credentials_frame, textvariable=self.username_var, width=25)
        self.username_entry.grid(row=1, column=0, sticky=(tk.W, tk.E), pady=(0, 10))
        
        ttk.Label(self.credentials_frame, text="Contraseña:").grid(row=2, column=0, sticky=tk.W, pady=(0, 5))
        self.password_entry = ttk.Entry(self.credentials_frame, textvariable=self.password_var, 
                                       show="*", width=25)
        self.password_entry.grid(row=3, column=0, sticky=(tk.W, tk.E), pady=(0, 10))
    ...
    def _load_saved_config(self):
        ...
        try:
            config_file = os.path.join(os.path.dirname(__file__), '..', '..', 'config', 'connection.json')
            ...
            if os.path.exists(config_file):
                ...
                self.database_var.set(config.get('database', 'ExcelSQLIntegration'))
            ...
            self.logger.warning(f"No se pudo cargar configuración guardada: {str(e)}")
