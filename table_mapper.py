...
class TableMapper:
    ...
    def _load_table_schemas(self):
        ...
        try:
            ...
            for row in results:
                ...
                mapped_type = self.data_type_mapping.get(sql_type, DataType.STRING)
                ...
                if not column_info['is_nullable']:
                    self.table_schemas[table_key].not_null_columns.append(row['COLUMN_NAME'])
            
            # Cargar información de claves primarias y restricciones
            self._load_table_constraints()
            
            self.logger.info(f"Cargados esquemas de {len(self.table_schemas)} tablas")
        ...
        try:
            ...
            for row in pk_results:
                ...
                if table_key in self.table_schemas:
                    self.table_schemas[table_key].primary_keys.append(row['COLUMN_NAME'])
            ...
            for row in unique_results:
                table_key = f"{row['TABLE_SCHEMA']}.{row['TABLE_NAME']}"
                if table_key in self.table_schemas:
                    self.table_schemas[table_key].unique_constraints.append(row['COLUMN_NAME'])
            ...
            self.logger.warning(f"Error cargando restricciones de tablas: {str(e)}")
    ...
    def suggest_table_mapping(self, worksheet_name: str, 
        ...
        for table_key, schema in self.table_schemas.items():
            ...
            name_similarity = fuzz.ratio(worksheet_name.lower(), schema.table_name.lower()) / 100
            
            # Calcular similitud de columnas
            column_similarity = self._calculate_column_similarity(excel_columns, schema.get_column_names())
            
            # Score combinado (60% columnas, 40% nombre)
            combined_score = (column_similarity * 0.6) + (name_similarity * 0.4)
    ...
    def _calculate_column_similarity(self, excel_columns: List[str], 
        ...
        for excel_col in excel_columns:
            ...
            for db_col in db_columns:
                ...
                # Promedio ponderado
                combined_score = (ratio_score * 0.5 + partial_score * 0.3 + token_sort_score * 0.2)
    ...
    def validate_mapping(self, worksheet_mapping: WorksheetMapping) -> Dict[str, Any]:
        ...
        mapped_db_columns = {m.db_column for m in worksheet_mapping.column_mappings}
